   ...
   public function destroy(Report $report)
{
    if (Auth::user()->id !== $report->user_id) {
        abort(403, 'вас нет прав на удаление этой записи.');
    }
    ...
    // Добавляем дополнительные поля к $data
        $data['user_id'] = Auth::user()->id;  // ID текущего пользователя
        $data['status_id'] = 1;               // Статус "новый" (ID = 1)
    ...
    public function edit(Report $report){
        if (Auth::user()->id === $report->user_id) {
            ...
            abort(403, 'вас нет прав на редактирование этой записи.');
        }
    }
